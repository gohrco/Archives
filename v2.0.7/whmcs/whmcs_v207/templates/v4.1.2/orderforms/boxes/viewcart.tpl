...
<tr class="clientareatableactive"><td align="left">
...
<tr class="clientareatableactive"><td><strong>{$LANG.orderaddon}</strong> - {$addon.name}</td><td align="center"><strong>{$addon.pricingtext}</strong></td></tr>
{/foreach}
<tr class="clientareatableactive"><td align="left"><a href="{$smarty.server.PHP_SELF}?a=confproduct&i={$num}" style="color:#009900;">[{$LANG.carteditproductconfig}]</a></td><td>&nbsp;</td></tr>
{/foreach}
...
<tr class="clientareatableactive"><td align="left"><a href="{$smarty.server.PHP_SELF}?a=confdomains" style="color:#009900;">[{$LANG.cartconfigdomainextras}]</a></td><td>&nbsp;</td></tr>
{/foreach}
...
<tr class="clientareatableheading"><td align="right">{$LANG.ordersubtotal}: &nbsp;</td><td align="center">{$subtotal}</td></tr>
{if $promotioncode}
<tr class="clientareatableheading"><td align="right">{$promotiondescription}: &nbsp;</td><td align="center">{$discount}</td></tr>
{/if}
{if $taxrate}
<tr class="clientareatableheading"><td align="right">{$taxname} @ {$taxrate}%: &nbsp;</td><td align="center">{$taxtotal}</td></tr>
{/if}
{if $taxrate2}
<tr class="clientareatableheading"><td align="right">{$taxname2} @ {$taxrate2}%: &nbsp;</td><td align="center">{$taxtotal2}</td></tr>
{/if}
<tr class="clientareatableheading"><td align="right">{$LANG.ordertotalduetoday}: &nbsp;</td><td align="center">{$total}</td></tr>
...
<tr class="clientareatableheading"><td align="right">{$LANG.ordertotalrecurring}: &nbsp;</td><td align="center">{if $totalrecurringmonthly}{$totalrecurringmonthly} {$LANG.orderpaymenttermmonthly}<br />{/if}
...
{if $promotioncode}{$promotioncode} - {$promotiondescription}<br /><a href="{$smarty.server.PHP_SELF}?a=removepromo">{$LANG.orderdontusepromo}</a>{else}<input type="text" name="promocode" size="15" /><br /><input type="submit" value="{$LANG.orderpromovalidatebutton}" />{/if}
</form>
</td><td width="45%" align="center">
<input type="button" value="&laquo; {$LANG.orderstartover}" onclick="window.location='cart.php?a=startover'" /> &nbsp;&nbsp; <input type="button" value="{$LANG.checkout} &raquo; " onclick="window.location='cart.php?a=checkout'"{if $cartitems==0} disabled{/if} />
</td></tr></table>
