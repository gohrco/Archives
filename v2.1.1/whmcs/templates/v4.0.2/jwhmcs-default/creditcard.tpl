...
<p><strong>&nbsp;&raquo;&nbsp;&nbsp;{$LANG.creditcardyourinfo}</strong></p>
...
<p><strong>&nbsp;&raquo;&nbsp;&nbsp;{$LANG.creditcarddetails}</strong></p>

<p><input type="radio" name="ccinfo" value="useexisting" id="useexisting"{if $cardnum} checked{else} disabled{/if} onClick="document.getElementById('ccinfonew').style.display='none';document.getElementById('ccinfoexisting').style.display='';" /> <label for="useexisting">{$LANG.creditcarduseexisting}{if $cardnum} ({$cardnum}){/if}</label><br />
<input type="radio" name="ccinfo" value="new" id="new"{if !$cardnum} checked{/if} onClick="document.getElementById('ccinfonew').style.display='';document.getElementById('ccinfoexisting').style.display='none';" /> <label for="new">{$LANG.creditcardenternewcard}</label></p>

<div id="ccinfoexisting" style="display:none">
<table cellspacing="1" cellpadding="0" class="frame"><tr><td>
<table width="100%" cellpadding="2">
<tr><td width=120 class="fieldarea">{$LANG.creditcardcvvnumber}</td><td><input type="text" name="cccvv2" value="{$cccvv}" size="5"> <a href="#" onclick="window.open('images/ccv.gif','','width=280,height=200,scrollbars=no,top=100,left=100');return false">{$LANG.creditcardcvvwhere}</a></td></tr>
</table>
</td></tr></table>
</div>
...
<table cellspacing="1" cellpadding="0" class="frame"><tr><td>
<table width="100%" cellpadding="2">
<tr><td width=120 class="fieldarea">{$LANG.creditcardcardtype}</td><td><select name="cctype">
{foreach key=num item=cardtype from=$acceptedcctypes}
<option{if $cctype eq "{$cardtype}"} selected{/if}>{$cardtype}</option>
{/foreach}
</select></td></tr>
<tr><td class="fieldarea">{$LANG.creditcardcardnumber}</td><td><input type="text" name="ccnumber" size="30" value="{$ccnumber}"></td></tr>
<tr><td class="fieldarea">{$LANG.creditcardcardexpires}</td><td><input type="text" name="ccexpirymonth" size="2" maxlength="2" value="{$ccexpirymonth}">/<input type="text" name="ccexpiryyear" size="2" maxlength="2" value="{$ccexpiryyear}"> (MM/YY)</td></tr>
...
<p align="center"><input type="submit" value="{$LANG.ordercontinuebutton}" onclick="this.value='{$LANG.pleasewait}'" /></p>
